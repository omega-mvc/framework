...
use Omega\DocBlockGenerator\VarExport\Compiler\ClosureCompiler;
use Omega\DocBlockGenerator\VarExport\Compiler\StringCompiler;
...
final class VarExport
{
    ...
    {
        $this->stringCompiler = new StringCompiler();
    }
     ...
     */
    public function compile(array $data, string $fileName): bool
    {
        ...
        $result = file_put_contents($fileName, $this->compileToString($data));
    ...
    {
        $this->addToBuffers(
            $this->stringCompiler->compile($string)
        );
    }
    ...
    private function writeArrayElements(array $elements, bool $isArrayMode = true): void
    {
        ...
        foreach ($elements as $key => $value) {
            ...
            if ($isArrayMode && $this->alignArray) {
                ...
                $this->addToBuffer(str_repeat(' ', max(0, ($keyLength - $length) + $this->indentLevel)));
