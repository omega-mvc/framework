...
class ZipAdapter implements AdapterInterface
{
    ...
    /**
     * Constructor that initializes the ZipArchive and opens the specified ZIP file, creating it if it does not exist.
     *
    ...
    /**
     * {@inheritdoc}
     * @throws RuntimeException if unable to open the ZIP file.
     */
    ...
    {
        $this->zipArchive->addFromString($key, $content);
    ...
    public function keys(): array
    {
        ...
        for ($i = 0; $i < $this->zipArchive->numFiles; ++$i) {
            $keys[] = (string) $this->zipArchive->getNameIndex($i);
        }
    ...
    {
        if ($this->zipArchive->locateName($sourceKey) === false) {
            throw new RuntimeException("The source file '$sourceKey' does not exist in the ZIP archive.");
        }

        if ($this->zipArchive->locateName($targetKey) !== false) {
