<?php

declare(strict_types=1);

namespace Tests\Database\RealDatabase;

use Omega\Database\Query\Query;
use Omega\Database\Query\Join\InnerJoin;
use Tests\Database\ManagesDatabase;

uses(ManagesDatabase::class);

covers(Query::class);
covers(InnerJoin::class);

afterEach(function (): void {
    $this->dropConnection();
});

test('it can join in select query', function (string $engine): void {
    $this->requiresOneOf($engine, ['mysql', 'mariadb'], 'AUTO_INCREMENT columns');

    $this->createConnection($engine);
    $this->pdo->query('CREATE TABLE users (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(100) NOT NULL,
        email VARCHAR(100) UNIQUE NOT NULL,
        role_id INT NOT NULL
    )')->execute();
    $this->pdo->query('CREATE TABLE roles (
        id INT AUTO_INCREMENT PRIMARY KEY,
        role_name VARCHAR(100) NOT NULL UNIQUE
    )')->execute();
    $this->pdo->query('CREATE TABLE logs (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        action VARCHAR(255) NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id)
    )')->execute();

    $this->pdo->query('INSERT INTO roles (role_name) VALUES
        ("Admin"),
        ("Editor"),
        ("Subscriber")')->execute();
    $this->pdo->query('INSERT INTO users (name, email, role_id) VALUES
        (\'Alice\', \'alice@example.com\', 1),
        (\'Bob\', \'bob@example.com\', 2),
        (\'Charlie\', \'charlie@example.com\', 3);')->execute();
    $this->pdo->query('INSERT INTO logs (user_id, action) VALUES
        (1, \'Logged In\'),
        (2, \'Logged In\'),
        (1, \'Deactivated\'),
        (3, \'Logged Out\');')->execute();

    $users = Query::from('users', $this->pdo)
        ->select(['users.name', 'roles.role_name'])
        ->join(InnerJoin::ref('roles', 'role_id', 'id'))
        ->all() ?: [];

    
    expect($users[0]['name'])->toEqual('Alice');
    expect($users[0]['role_name'])->toEqual('Admin');
})->with(ManagesDatabase::engineProvider());

test('it can join in update query', function (string $engine): void {
    $this->requiresOneOf($engine, ['mysql', 'mariadb'], 'AUTO_INCREMENT columns');

    $this->createConnection($engine);
    $this->pdo->query('CREATE TABLE users (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(100) NOT NULL,
        email VARCHAR(100) UNIQUE NOT NULL,
        role_id INT NOT NULL
    )')->execute();
    $this->pdo->query('CREATE TABLE roles (
        id INT AUTO_INCREMENT PRIMARY KEY,
        role_name VARCHAR(100) NOT NULL UNIQUE
    )')->execute();
    $this->pdo->query('CREATE TABLE logs (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        action VARCHAR(255) NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id)
    )')->execute();

    $this->pdo->query('INSERT INTO roles (role_name) VALUES
        ("Admin"),
        ("Editor"),
        ("Subscriber")')->execute();
    $this->pdo->query('INSERT INTO users (name, email, role_id) VALUES
        (\'Alice\', \'alice@example.com\', 1),
        (\'Bob\', \'bob@example.com\', 2),
        (\'Charlie\', \'charlie@example.com\', 3);')->execute();
    $this->pdo->query('INSERT INTO logs (user_id, action) VALUES
        (1, \'Logged In\'),
        (2, \'Logged In\'),
        (1, \'Deactivated\'),
        (3, \'Logged Out\');')->execute();

    Query::from('users', $this->pdo)
        ->update()
        ->value('name', 'Eve')
        ->join(InnerJoin::ref('roles', 'role_id', 'id'))
        ->equal('roles.role_name', 'Admin')
        ->execute();

    $users = $this->pdo->query('
        SELECT
            users.name, roles.role_name
        FROM users
        INNER JOIN roles ON
            users.role_id = roles.id
        ')
        ->resultset() ?: [];

    
    expect($users[0]['name'])->toEqual('Eve');
    expect($users[0]['role_name'])->toEqual('Admin');
})->with(ManagesDatabase::engineProvider());

test('it can join in delete query', function (string $engine): void {
    $this->requiresOneOf($engine, ['mysql', 'mariadb'], 'AUTO_INCREMENT columns');

    $this->createConnection($engine);
    $this->pdo->query('CREATE TABLE users (
        id INT AUTO_INCREMENT PRIMARY KEY,
        name VARCHAR(100) NOT NULL,
        email VARCHAR(100) UNIQUE NOT NULL,
        role_id INT NOT NULL
    )')->execute();
    $this->pdo->query('CREATE TABLE roles (
        id INT AUTO_INCREMENT PRIMARY KEY,
        role_name VARCHAR(100) NOT NULL UNIQUE
    )')->execute();
    $this->pdo->query('CREATE TABLE logs (
        id INT AUTO_INCREMENT PRIMARY KEY,
        user_id INT NOT NULL,
        action VARCHAR(255) NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (user_id) REFERENCES users(id)
    )')->execute();

    $this->pdo->query('INSERT INTO roles (role_name) VALUES
        ("Admin"),
        ("Editor"),
        ("Subscriber")')->execute();
    $this->pdo->query('INSERT INTO users (name, email, role_id) VALUES
        (\'Alice\', \'alice@example.com\', 1),
        (\'Bob\', \'bob@example.com\', 2),
        (\'Charlie\', \'charlie@example.com\', 3);')->execute();
    $this->pdo->query('INSERT INTO logs (user_id, action) VALUES
        (1, \'Logged In\'),
        (2, \'Logged In\'),
        (1, \'Deactivated\'),
        (3, \'Logged Out\');')->execute();

    // Delete related logs first
    Query::from('logs', $this->pdo)
        ->delete()
        ->alias('l')
        ->join(InnerJoin::ref('users', 'user_id', 'id'))
        ->equal('users.role_id', 1) // Assuming role_id 1 is for 'Admin'
        ->execute();

    Query::from('users', $this->pdo)
        ->delete()
        ->alias('u')
        ->join(InnerJoin::ref('roles', 'role_id', 'id'))
        ->equal('roles.role_name', 'Admin')
        ->execute();

    $users = $this->pdo->query('
        SELECT
            users.name, roles.role_name
        FROM users
        INNER JOIN roles ON
            users.role_id = roles.id
        ')
        ->resultset() ?: [];

    
    expect($users[0]['name'])->toEqual('Bob');
    expect($users[0]['role_name'])->toEqual('Editor');
})->with(ManagesDatabase::engineProvider());